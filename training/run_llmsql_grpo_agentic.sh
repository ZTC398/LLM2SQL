#!/usr/bin/env bash
# GRPO + agentic RL | LLMSQL | single-node launcher for verl
...
DATA_DIR=${DATA_DIR:-/root/shared-nvme/rlvr/verl_data/llmsql_agent_0shot}
...
AGENT_LOOP_CONFIG_PATH=${AGENT_LOOP_CONFIG_PATH:-${REPO_ROOT}/configs/agent_loops/llmsql_tool_agent.yaml}
DEFAULT_AGENT_LOOP=${DEFAULT_AGENT_LOOP:-llmsql_tool_agent}
...
PROJECT_NAME=${PROJECT_NAME:-llmsql_grpo_agentic}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen25_coder_3b_agent_0shot}
DEFAULT_LOCAL_DIR=${DEFAULT_LOCAL_DIR:-${OUTPUT_ROOT}/${PROJECT_NAME}/${EXPERIMENT_NAME}}

mkdir -p "${DEFAULT_LOCAL_DIR}"

if [[ -n "${TOTAL_TRAINING_STEPS}" ]]; then
    EFFECTIVE_TOTAL_EPOCHS=${EFFECTIVE_TOTAL_EPOCHS:-1000000}
else
    EFFECTIVE_TOTAL_EPOCHS=${EFFECTIVE_TOTAL_EPOCHS:-${TOTAL_EPOCHS}}
fi
...
REWARD=(
    ...
    reward.custom_reward_function.path="${REPO_ROOT}/rewards/llmsql_agent_reward.py"
