#!/usr/bin/env bash
# GRPO + full fine-tuning | LLMSQL | single-node launcher for verl
...
PROJECT_NAME=${PROJECT_NAME:-llmsql_grpo_full}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-qwen25_coder_3b_5shot_full}
DEFAULT_LOCAL_DIR=${DEFAULT_LOCAL_DIR:-${OUTPUT_ROOT}/${PROJECT_NAME}/${EXPERIMENT_NAME}}

mkdir -p "${DEFAULT_LOCAL_DIR}"

if [[ -n "${TOTAL_TRAINING_STEPS}" ]]; then
    # trainer.total_training_steps is checked inside the train loop, but
    # trainer.total_epochs is still the outer loop bound. Keep epoch count
    # effectively unbounded when explicit step control is requested.
